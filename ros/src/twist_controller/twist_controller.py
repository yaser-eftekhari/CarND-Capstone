...
class Controller(object):
    def __init__(self, wheel_base, steer_ratio, min_speed, max_lat_accel, max_steer_angle):
        ...
        self.breakPID = PID(.5, .01, .1, mn=0, mx=.5)
    ...
    def control(self, target_lin_vel, target_ang_vel, current_lin_vel):
        ...
        if (current_lin_vel-target_lin_vel) > -1:
            break_pid = self.breakPID.step( (current_lin_vel-target_lin_vel+1), .05)
            throttle_pid = max(0,1 - abs(steer_pid) - self.throttlePID.step( (current_lin_vel-target_lin_vel+1), .05))
        else:
            break_pid = 0
